...
public interface DesktopIntegration : GLib.Object {
    ...
    public static async DesktopIntegration? get_instance () {
        if (instance == null) {
            try {
                instance = yield Bus.get_proxy (SESSION, "org.pantheon.gala", "/org/pantheon/gala/DesktopInterface");
            } catch (Error e) {
                warning ("Failed to get desktop integration dbus proxy: %s", e.message);
            }
        }
